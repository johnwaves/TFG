...
const TratamientoRegistroSanitario = () => {
    ...
    const fetchTratamientos = async (dniPaciente) => {
        try {
            ...
            if (response.ok) {
                ...
                const noFarmacologicosActivos = data.filter(tratamiento => {
                    const endDate = new Date(tratamiento.fecha_fin) 
                    return (
                        tratamiento.tipo === "NO_FARMACOLOGICO" &&
                        (!tratamiento.fecha_fin || endDate >= today)
                    ) 
                }) 
                if (noFarmacologicosActivos.length === 0) {
                    setShowNoTratamientosDialog(true) 
                } else {
                    setTratamientos(noFarmacologicosActivos) 
                }
            } else {
    ...
    const handleTratamientoChange = async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                if (data.lastRegistroDate) {
                    const lastRegistroDate = new Date(data.lastRegistroDate) 
                    const today = new Date() 

                    if (
                        lastRegistroDate.getDate() === today.getDate() &&
                        lastRegistroDate.getMonth() === today.getMonth() &&
                        lastRegistroDate.getFullYear() === today.getFullYear()
                    ) {
                        const nextAvailableDate = new Date(today) 
                        nextAvailableDate.setHours(23, 59, 59, 999) 
            ...
            <div className="flex items-center justify-center m-10">
                <div className="card bg-base-100 w-full max-w-xl shadow-2xl p-8">
                    <form onSubmit={handleSubmit} className="flex flex-col items-center">
                        ...
                        {nextRegistro ? (
                            <>
                                <p className="text text-center mb-4">Tiempo restante hasta el próximo registro:</p>
                                {renderCountdown()}
...
export default TratamientoRegistroSanitario 
